...
import { generateContent, generateContentStream, isInitialized } from '../services/geminiService.js';
...
const handleChat = async (req, res, availableFunctions, systemInstructionText, tools) => {
  ...
  try {
    ...
    console.log(`[${requestId}] Chat History prepared with ${sessionHistory.length} messages from session`);

    console.log(`[${requestId}] Sending message to Gemini (Chat) with history length ${sessionHistory.length}...`);
    ...
    while (functionCallCount < MAX_FUNCTION_CALLS) {
      ...
      functionCallCount++;
      console.log(`[${requestId}] Processing potential function call iteration ${functionCallCount}/${MAX_FUNCTION_CALLS}`);
      ...
      if (responseFromModel.candidates?.[0]?.content?.parts) {
        ...
        console.warn(`[${requestId}] No valid parts received from model's response (Chat) in iteration ${functionCallCount}.`);
        try { 
      ...
      if (functionCalls && functionCalls.length > 0) {
        ...
        console.log(`[${requestId}] **** Detected Function Call (Chat) - Iteration ${functionCallCount}: ****`); 
        ...
        if (functionToExecute) {
          try {
            console.log(`[${requestId}] Executing function: ${functionCall.name} with args:`, functionCall.args);
            const executionResult = await functionToExecute(functionCall.args);
            functionResultPayload = executionResult.result;
            console.log(`[${requestId}] Function ${functionCall.name} executed successfully. Result:`, functionResultPayload);
          } catch (funcError) {
            console.error(`[${requestId}] Error executing function ${functionCall.name}:`, funcError);
        ...
        console.log(`[${requestId}] Sending function response back to Gemini (Chat) with history length ${updatedHistory.length}... (Iteration ${functionCallCount})`);
        ...
        if (!responseFromModel) { 
          throw new Error(`Invalid response structure from AI service after function call (iteration ${functionCallCount}).`); 
        }
        ...
        if (containsOnlyFunctionCalls) {
          ...
          console.log(`[${requestId}] Response contains only function calls, continuing loop (Iteration ${functionCallCount})`);
          continue;
        } else {
          // Se contém texto final, extraímos o texto e quebramos o loop
          console.log(`[${requestId}] Response contains final text, breaking loop after ${functionCallCount} function calls`);
          modelResponseText = extractModelResponseText(responseFromModel);
          break;
        }
      } else {
        // Nenhuma chamada de função detectada, apenas resposta de texto
        console.log(`[${requestId}] Gemini responded directly with text (Chat) in iteration ${functionCallCount}.`);
        break;
      }
    }
    ...
    if (functionCallCount >= MAX_FUNCTION_CALLS) {
      console.warn(`[${requestId}] Reached maximum function call limit (${MAX_FUNCTION_CALLS}). Finishing sequence.`);
    }
    ...
    if (!res.headersSent) { 
      res.status(500).json({ error: `Error processing request: ${error.message || 'Unknown error'}` }); 
    } else { 
...
const handleStreamChat = async (req, res, availableFunctions, systemInstructionText, tools) => {
  ...
  try {
    ...
    // Obtém o histórico atual da conversa
    const sessionHistory = conversationMemory.getHistory(sessionId);
    console.log(`[${requestId}] Stream History prepared with ${sessionHistory.length} messages from session`);

    console.log(`[${requestId}] Starting stream request to Gemini with history length: ${sessionHistory.length}`);
    ...
    const processRemainingFunctions = async (history, functionCalls, currentIteration = 1) => {
      ...
      console.log(`[${requestId}] Processing function call #${currentIteration + 1}: ${nextFunctionCall.name}`);
      ...
      conversationMemory.addFunctionCall(sessionId, nextFunctionCall.name, nextFunctionCall.args || {});
      
      // Verificar argumentos
      if (!nextFunctionCall.args){
        console.error(`[${requestId}] Function call detected but args are missing in pre-collected call!`, nextFunctionCall);
        res.write(JSON.stringify({ 
          type: 'error', 
      ...
      if (functionToExecute) {
        try {
          ...
          console.log(`[${requestId}] Function executed successfully. Result:`, functionResultPayload);
        } catch (funcError) {
      ...
      console.log(`[${requestId}] **** Sending Function Result to Client (sequence #${currentIteration + 1}): ****`); 
    ...
    const processStreamFunctionCalls = async (history, functionCallCount = 0) => {
      ...
      if (functionCallCount >= MAX_FUNCTION_CALLS) {
        console.warn(`[${requestId}] Reached maximum function call limit (${MAX_FUNCTION_CALLS}) in streaming mode.`);
        res.write(JSON.stringify({ 
          type: 'info', 
      ...
      functionCallCount++;
      console.log(`[${requestId}] Processing streaming function call iteration ${functionCallCount}/${MAX_FUNCTION_CALLS}`); 
      ...
      console.log(`[${requestId}] Established stream connection for function iteration ${functionCallCount}.`);
      ...
      for await (const chunk of iterationStreamResult) {
        if (!chunk) { 
          console.warn(`[${requestId}] Received null/undefined chunk in iteration ${functionCallCount}.`); 
          continue; 
        }

        const candidate = chunk?.candidates?.[0];
        if (!candidate) { 
          console.warn(`[${requestId}] Chunk received without candidates in iteration ${functionCallCount}:`, chunk); 
          continue; 
        }
        ...
        if (currentContentParts) {
          for (const part of currentContentParts) {
            ...
            if (part.text) {
              console.log(`[${requestId}] Streaming text chunk (iteration ${functionCallCount}): "${part.text.substring(0, 50)}..."`);
            ...
            if (part.functionCall && !iterationFunctionCall) {
              console.log(`[${requestId}] **** Detected Function Call (Stream Part) in iteration ${functionCallCount}: ****`); 
          ...
          console.log(`[${requestId}] Chunk received without content parts in iteration ${functionCallCount}:`, chunk); 
        }
      }
      ...
      if (iterationTextResponse && iterationHasSignificantText && !iterationFunctionCall) {
        ...
        console.log(`[${requestId}] Added model text response to session history (iteration ${functionCallCount}, no function call).`);
        return; // Finaliza o processamento recursivo
      }
      // Se tiver uma chamada de função, processa ela
      else if (iterationFunctionCall) {
        // Adiciona a chamada de função ao histórico da sessão
        conversationMemory.addFunctionCall(sessionId, iterationFunctionCall.name, iterationFunctionCall.args || {});
        
        if (!iterationFunctionCall.args){
          console.error(`[${requestId}] Function call detected but args are missing in iteration ${functionCallCount}!`, iterationFunctionCall);
          res.write(JSON.stringify({ 
            type: 'error', 
            data: 'Erro interno: Argumentos faltando para a função.' 
          }) + '\n');
        ...
        console.log(`[${requestId}] Handling function call in iteration ${functionCallCount}: ${iterationFunctionCall.name} with args:`, iterationFunctionCall.args);
        let functionExecutionResult;
        let functionResultPayload;
        ...
        if (functionToExecute) {
          try {
            console.log(`[${requestId}] Executing function in iteration ${functionCallCount}: ${iterationFunctionCall.name}`);
            functionExecutionResult = await functionToExecute(iterationFunctionCall.args);
            functionResultPayload = functionExecutionResult.result;
            console.log(`[${requestId}] Function executed in iteration ${functionCallCount}. Result payload:`, functionResultPayload);
          } catch (funcError) {
            console.error(`[${requestId}] Error executing function in iteration ${functionCallCount}:`, funcError);
            const errorPayload = { 
              success: false, 
          ...
          console.warn(`[${requestId}] Unknown function call requested in iteration ${functionCallCount}: ${iterationFunctionCall.name}`);
          const errorPayload = { 
            success: false, 
        ...
        // Envia o resultado para o cliente
        console.log(`[${requestId}] **** Sending Function Result to Client (iteration ${functionCallCount}): ****`); 
        console.dir(functionExecutionResult.result, { depth: null });
        ...
        conversationMemory.addFunctionResult(sessionId, iterationFunctionCall.name, functionResultPayload);
        ...
        if (iterationHasSignificantText) {
          if (iterationTextResponse) {
            conversationMemory.addModelTextResponse(sessionId, iterationTextResponse);
            console.log(`[${requestId}] Added mixed text/function response to session history (iteration ${functionCallCount}).`);
          }
          return; // Finaliza o processamento recursivo
        }
        
        // Continuar o processamento recursivo com o histórico atualizado
        console.log(`[${requestId}] Continuing function call sequence with iteration ${functionCallCount + 1}. History length: ${updatedHistory.length}`);
        ...
        console.log(`[${requestId}] No significant content or function calls detected in iteration ${functionCallCount}. Ending sequence.`);
        return;
      }
    };
    ...
    for await (const chunk of initialStreamResult) {
      ...
      if (currentContentParts) {
        for (const part of currentContentParts) {
          // Stream texto para o cliente
          if (part.text) {
            console.log(`[${requestId}] Streaming initial text chunk: "${part.text.substring(0, 50)}..."`);
            res.write(JSON.stringify({ type: 'text', data: part.text }) + '\n');
          ...
          if (part.functionCall) {
            ...
            if (!initialFunctionCall) {
              ...
              console.log(`[${requestId}] Detected additional function call in same turn: ${part.functionCall.name}`); 
            }
          }
    ...
    console.log(`[${requestId}] Found ${pendingFunctionCalls.length} function calls in initial turn.`);
    ...
    if (initialTextResponse && initialHasSignificantText && pendingFunctionCalls.length === 0) {
      ...
      console.log(`[${requestId}] Added model text response to session history (no function call).`);
    }
    // Se tiver chamadas de função, processa todas elas em sequência
      ...
      // Adiciona a chamada de função ao histórico da sessão
      conversationMemory.addFunctionCall(sessionId, initialFunctionCall.name, initialFunctionCall.args || {});
      
      if (!initialFunctionCall.args){
        console.error(`[${requestId}] Function call detected but args are missing!`, initialFunctionCall);
        res.write(JSON.stringify({ 
          type: 'error', 
          data: 'Erro interno: Argumentos faltando para a função.' 
        }) + '\n');
      } else {
        console.log(`[${requestId}] Handling initial function call: ${initialFunctionCall.name} with args:`, initialFunctionCall.args);
        let functionExecutionResult;
        let functionResultPayload;
        ...
        if (functionToExecute) {
          ...
          console.warn(`[${requestId}] Unknown function call requested: ${initialFunctionCall.name}`);
          const errorPayload = { 
            success: false, 
        ...
        // Adiciona o resultado ao histórico da sessão
        conversationMemory.addFunctionResult(sessionId, initialFunctionCall.name, functionResultPayload);
        
        // Obtém o histórico atualizado com a chamada de função e o resultado
        const updatedHistory = conversationMemory.getHistory(sessionId);
        
        // Se houver mais funções para processar ou se não tiver texto significativo, continuamos com sequência
        const shouldContinueSequence = pendingFunctionCalls.length > 1 || !initialHasSignificantText;
        
        if (shouldContinueSequence) {
          console.log(`[${requestId}] Continuing with function call sequence. Remaining calls: ${pendingFunctionCalls.length - 1}`);
          ...
          if (remainingCalls.length > 0) {
            console.log(`[${requestId}] Processing ${remainingCalls.length} additional function calls...`);
          ...
          console.log(`[${requestId}] Sending function response back to Gemini for final response... History length: ${updatedHistory.length}`);
    ...
    if (res && !res.headersSent) {
      ...
      console.error(`[${requestId}] Cannot send error - headers sent or stream ended/not writable.`); 
    }
  }
};
