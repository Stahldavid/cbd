  ...
  try {
    ...
    if (time_range && typeof time_range === 'string' && time_range.trim() !== '') {
      ...
      if (/^\d+[dwmy]$/.test(time_range.trim())) {
        ...
        console.warn(
          `[Tavily Search] Invalid time_range format received: ${time_range}. Ignoring.`
        );
      }
    }
    ...
    return {
      result: { success: false, error: `Error during Tavily search execution: ${error.message}` },
    };
  }
}
...
const tavilySearchDeclaration = {
  name: 'tavily_search',
  description:
    'Search the web for current information, news, articles, and research results using the Tavily search engine. Useful for recent events or information not in the training data.',
  parameters: {
    type: Type.OBJECT,
    properties: {
      query: {
      ...
      max_results: {
        type: Type.INTEGER,
        description:
          'Optional. The maximum number of search results to return (default 5, max 10).',
      },
      include_domains: {
        type: Type.ARRAY,
        items: { type: Type.STRING },
        description:
          'Optional. A list of domains to specifically include in the search results (e.g., ["pubmed.gov", "mayoclinic.org"])',
      },
      exclude_domains: {
        type: Type.ARRAY,
        items: { type: Type.STRING },
        description:
          'Optional. A list of domains to specifically exclude from the search results (e.g., ["wikipedia.org"])',
      },
      time_range: {
        type: Type.STRING,
        description:
          'Optional. The time range to filter results (e.g., "1d", "2w", "3m", "1y" for past day, 2 weeks, 3 months, 1 year).',
      },
    },
