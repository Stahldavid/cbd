...
async function fetchAndProcessUrl({
  ...
  console.log(
    `[FetchAndProcess] Attempting: ${urlString}, raw=${raw}, start=${startIndex}, max=${maxLength}`
  );
  let targetUrl;
  try {
  ...
  try {
    ...
    if (robotsResponse.status >= 200 && robotsResponse.status < 300) {
      ...
      return {
        result: {
          success: false,
          error: `Não foi possível verificar ${robotsUrl} (Status: ${robotsResponse.status}). Acesso negado por precaução.`,
        },
      };
    }
    if (robots && !robots.isAllowed(targetUrl.href, DEFAULT_USER_AGENT)) {
      return {
        result: { success: false, error: `Acesso a ${targetUrl.href} negado por ${robotsUrl}.` },
      };
    }
  } catch (error) {
    if (error.name === 'AbortError') {
      return { result: { success: false, error: `Timeout ao verificar ${robotsUrl}.` } };
    }
    return {
      result: {
        success: false,
        error: `Erro ao verificar permissões (${robotsUrl}): ${error.message}`,
      },
    };
  }
  ...
  try {
    ...
    if (error.name === 'AbortError') {
      return {
        result: { success: false, error: `Timeout ao buscar conteúdo de ${targetUrl.href}.` },
      };
    }
    return {
      result: { success: false, error: `Erro ao buscar URL (${targetUrl.href}): ${error.message}` },
    };
  }
  ...
  if (raw) {
    ...
    if (!isHtml) {
      return {
        result: {
          success: false,
          error: `Não é possível extrair Markdown. O conteúdo de ${targetUrl.href} não é HTML (Content-Type: ${contentType}). Use raw:true para conteúdo bruto.`,
        },
      };
    }
    try {
      ...
      if (!article || !article.content) {
        throw new Error('Readability falhou na extração.');
      }
      const turndownService = new TurndownService({ headingStyle: 'atx' });
      finalContent = turndownService.turndown(article.content);
      if (!finalContent.trim()) {
        console.warn('Markdown extraído está vazio.');
      }
    } catch (error) {
      return {
        result: { success: false, error: `Erro ao processar HTML para Markdown: ${error.message}` },
      };
    }
  }
  ...
  console.log(
    `[FetchAndProcess] Success. Returning content slice (start: ${startIndex}, max: ${maxLength}, actual: ${slicedContent.length}).`
  );
...
const fetchAndProcessUrlDeclaration = {
  ...
  description:
    'Busca o conteúdo de uma URL da web, opcionalmente converte o artigo principal para Markdown (padrão) ou retorna o conteúdo bruto (HTML, texto, etc.). Pode retornar apenas uma parte do conteúdo especificando início e tamanho máximo. Verifica permissões em robots.txt.',
  parameters: {
    type: Type.OBJECT,
    properties: {
      urlString: {
        // Corresponde ao parâmetro da função JS
        type: Type.STRING,
      ...
      startIndex: {
        type: Type.INTEGER,
        description:
          'Opcional. Índice baseado em zero do caractere inicial a retornar (padrão: 0).',
      },
      raw: {
        type: Type.BOOLEAN,
        description:
          'Opcional. Se true, retorna o conteúdo bruto da URL sem tentar extrair/converter para Markdown (padrão: false).',
      },
    },
