...
const conversationMemory = {
  ...
  addUserMessage(sessionId, message) {
    ...
    console.log(`[Memory] Added user message to session ${sessionId}, history length: ${session.history.length}`);
    return userTurn;
  },
  ...
  addModelTextResponse(sessionId, text) {
    ...
    session.lastUpdate = Date.now();
    console.log(`[Memory] Added model text response to session ${sessionId}, history length: ${session.history.length}`);
    return modelTurn;
  },
  ...
  addFunctionCall(sessionId, name, args) {
    ...
    session.lastUpdate = Date.now();
    console.log(`[Memory] Added function call (${name}) to session ${sessionId}, history length: ${session.history.length}`);
  ...
  addFunctionResult(sessionId, name, result) {
    ...
    session.lastUpdate = Date.now();
    console.log(`[Memory] Added function result (${name}) to session ${sessionId}, history length: ${session.history.length}`);
  ...
  cleanup(maxAgeMs = 1000 * 60 * 60 * 24) { // Por padrão 24 horas
    const now = Date.now();
    let count = 0;
