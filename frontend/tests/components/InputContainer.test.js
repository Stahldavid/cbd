...
import { InputContainer } from '../../src/components/InputContainer'; // Adjusted path
...
describe('InputContainer Component', () => {
  ...
  test('send button is disabled when input is empty or only whitespace', () => {
    ...
    rerender(
      <ThemeProvider theme={theme}>
        <InputContainer value="   " onChange={mockOnChange} onKeyDown={mockOnKeyDown} onSend={mockOnSend} disabled={false} />
      </ThemeProvider>
    );
    expect(sendButton).toBeDisabled();
  });
  ...
  test('textarea auto-resizes (conceptual check, actual resize is visual)', () => {
    renderComponent();
    const textarea = screen.getByPlaceholderText(/Digite sua pergunta.../i); // Removed 'as HTMLTextAreaElement'
