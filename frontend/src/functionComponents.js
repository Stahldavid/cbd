  ...
  return (
    <FunctionCallContainer>
      <div>
        ...
        {showIterationIndicator && (
          <SequentialFunctionIndicator>Iteração {iteration}</SequentialFunctionIndicator>
        )}
      </div>
...
export function FunctionResultDisplay({ functionResultInfo }) {
  ...
  const resultData = functionResultInfo.result.error
    ? { error: functionResultInfo.result.error }
    : functionResultInfo.result;
  ...
  return (
    <FunctionResultContainer className={isError ? 'error-result' : ''}>
      <div>
        <strong>
          {isError ? '⚠️' : '✅'} Resultado ({functionResultInfo.name}):
        </strong>
        {showIterationIndicator && (
          <SequentialFunctionIndicator>Iteração {iteration}</SequentialFunctionIndicator>
        )}
